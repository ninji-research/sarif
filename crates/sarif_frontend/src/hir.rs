...
impl ConstExpr {
    ...
    /// Evaluate the constant expression with given parameter bindings
    #[must_use]
    pub fn eval(&self, params: &BTreeMap<String, u32>) -> Option<u32> {
        match self {
            Self::Literal(value) => Some(*value),
            Self::Param(name) => params.get(name).copied(),
            Self::Add(left, right) => left.eval(params)?.checked_add(right.eval(params)?),
            Self::Sub(left, right) => left.eval(params)?.checked_sub(right.eval(params)?),
            Self::Mul(left, right) => left.eval(params)?.checked_mul(right.eval(params)?),
        }
    }
}
