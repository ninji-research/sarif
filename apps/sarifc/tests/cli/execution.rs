...
fn run_executes_text_builder_consistently() {
    assert_run_parity(
        "fn main() -> Text { let mut builder = text_builder_new(); builder = text_builder_append(builder, \"sa\"); builder = text_builder_append(builder, text_slice(\"sarif\", 2, 5)); text_builder_finish(builder) }",
        "sarif",
    );
}
...
fn wasm_build_rejects_text_builder_modules() {
    let path = temp_source(
        "fn main() -> Text { let mut builder = text_builder_new(); builder = text_builder_append(builder, \"sarif\"); text_builder_finish(builder) }",
    );
