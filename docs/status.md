...
- **F64 return type preservation in C codegen**: Fixed a critical bug where user-defined function calls returning `F64` were unconditionally cast to `(uint64_t)` in the C backend, reinterpreting floating-point bit patterns as integers. C codegen now correctly emits bare assignments for `F64` return values.
- **`text_index_contains` return type unified**: Fixed an inconsistency where the frontend typechecked it as returning `Bool` but JIT, C codegen, and bootstrap typechecked it as returning `I32`. All backends and bootstrap checkers are unified to typecheck and process its return value as `Bool`.
